import torch
import torch.nn as nn
import torchvision

class EdgeEnhancementAttention(nn.Module):
    def __init__(self, in_channels, kernel_size=3):
        super().__init__()
        
        # 边缘检测卷积层
        self.edge_conv = nn.Sequential(
            nn.Conv2d(in_channels, in_channels, kernel_size=3, padding=1, groups=in_channels),
            nn.BatchNorm2d(in_channels),
            nn.ReLU(inplace=True)
        )
        
        # 通道注意力机制
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.fc1 = nn.Linear(in_channels, in_channels // 16, bias=False)
        self.fc2 = nn.Linear(in_channels // 16, in_channels, bias=False)
        self.sigmoid = nn.Sigmoid()

        # 融合层
        self.fusion_conv = nn.Conv2d(in_channels * 2, in_channels, kernel_size=1)
        self.batch_norm = nn.BatchNorm2d(in_channels)

    def forward(self, x):
        # 边缘特征提取
        edge_features = self.edge_conv(x)

        # 通道注意力计算
        b, c, h, w = x.size()
        avg_out = self.avg_pool(x).view(b, c)
        attention = self.fc1(avg_out)
        attention = nn.ReLU(inplace=True)(attention)
        attention = self.fc2(attention)
        attention = self.sigmoid(attention).view(b, c, 1, 1)

        # 应用通道注意力
        enhanced_features = x * attention
        
        # 融合边缘特征和增强特征
        fused = torch.cat((enhanced_features, edge_features), dim=1)
        out = self.fusion_conv(fused)
        out = self.batch_norm(out)

        return out

class EEA(nn.Module):
    def __init__(self, in_channels, kernel_size=3, padding=1, dilation=1):
        super().__init__()
        self.kernel_size = kernel_size
        # 使用边缘增强注意力模块
        self.edge_attention = EdgeEnhancementAttention(in_channels)

        # 学习可变形卷积
        self.deform = torchvision.ops.DeformConv2d(
            in_channels=in_channels,
            out_channels=in_channels,
            kernel_size=kernel_size,
            padding=padding,
            groups=in_channels,
            dilation=dilation,
            bias=False
        )

        self.balance = nn.Sequential(
            nn.Conv2d(in_channels, in_channels, 1),
            nn.BatchNorm2d(in_channels)
        )

    def forward(self, x):
        # 应用边缘增强注意力模块
        enhanced_features = self.edge_attention(x)

        # 学习偏移量，确保偏移量形状为 (B, 2 * kernel_size * kernel_size, H, W)
        B, C, H, W = enhanced_features.size()
        offsets = torch.zeros(B, 2 * self.kernel_size * self.kernel_size, H, W, device=x.device)

        # 学习可变形卷积
        out = self.deform(enhanced_features, offsets)  # 传递计算得到的偏移量
        out = self.balance(out) * x  # 融合原始特征

        return out
