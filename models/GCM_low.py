...
class GCM(nn.Module):
    def __init__(self, input_channels=4):  # 输入通道进一步减少
        super().__init__()
        ...
        self.sc_h = nn.Sequential(
            nn.Conv2d(input_channels, input_channels, kernel_size=(3, 1), padding=(1, 0), groups=input_channels),  # 降低卷积大小
            nn.GELU()
        )
        self.sc_v = nn.Sequential(
            nn.Conv2d(input_channels, input_channels, kernel_size=(1, 3), padding=(0, 1), groups=input_channels),  # 降低卷积大小
            nn.GELU()
        )
    ...
    def forward_pgc(self, prompt_l, prompt_h, x_ori):
        ...
        # 动态权重
        dynamic_weights = self.linear_p(torch.mean(attn_weights_h_to_l + attn_weights_l_to_h, dim=1))
        ...
        p_norm = prompt_l + prompt_h + p_norm
...
if __name__ == "__main__":
    input_channels = 8  # Reduced input channels for low configuration
    model = GCM(input_channels)
    example_input = torch.randn(8, input_channels, 8, 8)  # Example input: batch size 8, 8 channels, 8x8 image
    output = model(example_input)
    print(output.shape)  # Should output shape: (8, 8, 8, 8)
